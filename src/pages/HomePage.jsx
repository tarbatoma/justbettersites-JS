...
const HomePage = ({ onMouseEnter, onMouseLeave }) => {
  ...
  // Parallax și efecte asupra scroll-ului (opțional)
  const { scrollYProgress } = useScroll();
  const y1 = useTransform(scrollYProgress, [0, 1], [0, -150]);
  const opacity = useTransform(scrollYProgress, [0, 0.2], [1, 0]);

  // Calculăm lățimea pentru Carousel în mod responsive
  ...
  // Determină dacă ecranul este desktop
  useEffect(() => {
  ...
  // GSAP pentru animații în Hero (dacă nu folosești HeroSection separat)
  useEffect(() => {
    if (heroRef.current) {
  ...
  // Variante de animație pentru elemente care apar în secțiuni
  ...
  // Date fictive pentru servicii, soluții și proces (înlocuiește cu datele tale)
  const services = [
    {
      icon: (
  ...
  const solutions = [ t('solutions.automation'), t('solutions.ai') ];
  
  const processSteps = [
    { number: '01', title: t('homepage.process.discovery.title'), description: t('homepage.process.discovery.description') },
    { number: '02', title: t('homepage.process.strategy.title'), description: t('homepage.process.strategy.description') },
    { number: '03', title: t('homepage.process.design.title'), description: t('homepage.process.design.description') },
    { number: '04', title: t('homepage.process.development.title'), description: t('homepage.process.development.description') },
    { number: '05', title: t('homepage.process.testing.title'), description: t('homepage.process.testing.description') },
    { number: '06', title: t('homepage.process.launch.title'), description: t('homepage.process.launch.description') }
  ];

  return (
    <div className="overflow-hidden">
      {/* HERO SECTION */}
      <section ref={heroRef} className="relative min-h-screen flex items-center justify-center overflow-hidden bg-white" data-scroll-section>
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-8 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <BlurText
              ...
              animateBy="words"
              direction="top"
              className="hero-title text-3xl sm:text-5xl md:text-6xl font-bold mb-4 md:mb-6 text-black"
            />
            <motion.p className="hero-subtitle text-base sm:text-lg md:text-xl text-gray-700 mb-4 md:mb-6 max-w-3xl mx-auto">
              {t('homepage.hero.subtitle')}
            </motion.p>
            <motion.div className="hero-cta flex flex-col sm:flex-row justify-center gap-4">
              <Link
                to="/services"
                className="px-8 py-3 bg-primary text-white rounded-full font-medium hover:bg-blue-700 transition-colors shadow-lg hover:shadow-xl transform hover:-translate-y-1"
              >
                {t('homepage.hero.cta.services')}
              </Link>
              <Link
                to="/contact"
                className="px-8 py-3 bg-white text-primary border border-primary rounded-full font-medium hover:bg-gray-50 transition-colors shadow-lg hover:shadow-xl transform hover:-translate-y-1"
              >
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-8">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 40 }} animate={servicesInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, ease: "easeOut" }}>
            ...
            <p className="text-base sm:text-lg md:text-xl text-gray-600 max-w-3xl mx-auto">
              {t('homepage.servicesIntro.description')}
            </p>
          </motion.div>
          ...
          <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8" variants={staggerContainerVariants} initial="hidden" animate={servicesInView ? "visible" : "hidden"}>
            {services.map((service, index) => (
              <motion.div key={index} className="bg-light p-6 rounded-2xl hover-lift flex flex-col justify-between" variants={fadeInUpVariants} custom={index} whileHover={{ y: -10, transition: { duration: 0.3 } }}>
      ...
      <section ref={solutionsRef} className="py-24 bg-white" data-scroll-section>
        <div className="container mx-auto px-4 sm:px-6 md:px-8">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 40 }} animate={solutionsInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, ease: "easeOut" }}>
            ...
            />
            <p className="text-base sm:text-lg md:text-xl text-gray-600 max-w-3xl mx-auto">
              {t('homepage.solutionsIntro.description')}
            </p>
          </motion.div>
          ...
          <motion.div className="grid grid-cols-1 sm:grid-cols-2 gap-8" variants={staggerContainerVariants} initial="hidden" animate={solutionsInView ? "visible" : "hidden"}>
            <motion.div className="flex flex-col items-center text-center border border-primary rounded-2xl p-6 hover:scale-105 hover:bg-primary/5 transition-transform cursor-pointer group" onClick={() => window.location.href = '/services'} variants={fadeInUpVariants} custom={0}>
              <img src="/ba.png" alt="Business Automation" className="w-12 h-12 mb-4 group-hover:scale-110 transition-transform" />
              <h3 className="text-base sm:text-lg md:text-xl font-semibold text-gray-800 group-hover:text-primary">
                {t('homepage.solutions.automation')}
              </h3>
              <p className="text-xs sm:text-sm text-gray-500 mt-2">
                {t('homepage.solutions.clickAutomation')}
              </p>
            </motion.div>
            ...
            <motion.div className="flex flex-col items-center text-center border border-primary rounded-2xl p-6 hover:scale-105 hover:bg-primary/5 transition-transform cursor-pointer group" onClick={() => window.location.href = '/services'} variants={fadeInUpVariants} custom={1}>
              <img src="/ai.png" alt="AI Solutions" className="w-12 h-12 mb-4 group-hover:scale-110 transition-transform" />
              <h3 className="text-base sm:text-lg md:text-xl font-semibold text-gray-800 group-hover:text-primary">
                {t('homepage.solutions.ai')}
              </h3>
              <p className="text-xs sm:text-sm text-gray-500 mt-2">
                {t('homepage.solutions.clickAi')}
              </p>
            </motion.div>
          </motion.div>
        </div>
      ...
      <section ref={processRef} className="relative py-24 bg-light overflow-hidden" data-scroll-section>
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-8 relative z-10">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 40 }} animate={processInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, ease: "easeOut" }}>
            <BlurText 
              text={t('homepage.processIntro.title')} 
              delay={150} 
              animateBy="words" 
              direction="top" 
            ...
            <p className="text-base sm:text-lg md:text-xl text-white max-w-3xl mx-auto">
              {t('homepage.processIntro.description')}
            </p>
          </motion.div>
          ...
          <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" variants={staggerContainerVariants} initial="hidden" animate={processInView ? "visible" : "hidden"}>
            {processSteps.map((step, index) => (
              <motion.div
                key={index}
                className="bg-white p-6 rounded-2xl hover-lift relative z-10"
                variants={fadeInUpVariants}
                custom={index}
                whileHover={{ y: -10, transition: { duration: 0.3 } }}
              >
                <span className="text-4xl font-bold text-primary/20">{step.number}</span>
                <h3 className="text-base sm:text-lg md:text-xl font-bold mt-4 mb-2">{step.title}</h3>
      ...
      <section className="py-24 bg-white" data-scroll-section>
        <div className="container mx-auto px-4 sm:px-6 md:px-8">
          <motion.div
            className="bg-primary rounded-3xl p-8 sm:p-12 md:p-20 text-center text-white"
            initial={{ opacity: 0, y: 40 }}
          ...
          >
            <BlurText 
              text={t('homepage.cta.title')} 
              delay={150} 
              animateBy="words" 
              direction="top" 
              className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 text-white" 
            />
            <p className="text-base sm:text-lg md:text-xl text-white/80 mb-6 max-w-3xl mx-auto">
              {t('homepage.cta.description')}
            </p>
            <motion.a
              href="/contact"
              className="px-6 sm:px-8 py-3 bg-white text-primary rounded-full font-medium hover:bg-gray-100 transition-colors shadow-lg hover:shadow-xl inline-block"
